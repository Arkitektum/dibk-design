...
export const Aligned: Story = {
    render: () => (
        <div style={row}>
            <InputField id="control-height-input" label="Input field" hideLabel value="Input field" onChange={() => {}} noMargin />
            <Select id="control-height-select" label="Select" hideLabel options={options} value="Option 1" onChange={() => {}} />
    ...
    play: async ({ canvasElement }: { canvasElement: HTMLElement }) => {
        ...
        const measured: [string, number][] = [
            ...
            ...canvas
                .getAllByRole("button")
                .map((button): [string, number] => [button.textContent?.trim() || "button", button.getBoundingClientRect().height])
        ];

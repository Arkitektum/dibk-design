...
const Step = ({ step, index, activeStepId, direction = "vertical" }: StepProps) => {
    ...
    if (step.link && Object.keys(step.link).length > 0) {
        return (
            <li className={baseClassName}>
                <RouterLink to={step.link} aria-current={ariaCurrent} className={style.wizardTopnavItemContent}>
                    {renderStepContent()}
                </RouterLink>
            </li>
        );
    ...
    return (
        <li className={baseClassName}>
            <span aria-current={ariaCurrent} className={style.wizardTopnavItemContent}>
                {renderStepContent()}
            </span>
        </li>
    );

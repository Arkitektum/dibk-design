...
const isTextLike = (type?: string) => !type || ["text", "search", "url", "email", "tel", "password", "number"].includes(type);
...
const InputField = ({
    ...
    const normalizedValueProps = (() => {
        ...
        if (type === "date") {
            ...
            const d = value === undefined && defaultValue !== undefined ? toDateInputValue(defaultValue) : undefined;
            return value !== undefined ? { value: v ?? "" } : defaultValue !== undefined ? { defaultValue: d } : {};
        }
        const toStringish = (v: unknown) => (v instanceof Date ? String(v.getTime()) : (v as string | number));
        if (value !== undefined) return { value: toStringish(value) as string | number };
        if (defaultValue !== undefined) return { defaultValue: toStringish(defaultValue) as string | number };
        return {};
    })();
    ...
    const inputProps: React.InputHTMLAttributes<HTMLInputElement> = {
        ...
        "aria-describedby":
            [hasErrors && errorMessage ? getErrorElementId() : null, caption ? captionId : null, ariaDescribedBy].filter(Boolean).join(" ").trim() ||
            undefined,
